...
function	ValueAnimation({
	...
	function	initZero(): void {
		const words = document.querySelectorAll<HTMLElement>(`.${identifier}`);
		...
		<>
			<div className={'text'}>
				<p className={'wordWrapper'}> 
					<span className={`wordLeft ${identifier}`}>{`${prefix ? `${prefix} ` : ''}0.00${suffix ? ` ${suffix}` : ''}`}</span>
					<span className={`wordLeft ${identifier}`}>{`${prefix ? `${prefix} ` : ''}${value}${suffix ? ` ${suffix}` : ''}`}</span>
				</p>
			</div>
		</>
	);
